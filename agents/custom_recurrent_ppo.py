...
from sb3_contrib.common.recurrent.buffers import RecurrentDictRolloutBuffer, RecurrentRolloutBuffer, create_sequencers, pad, pad_and_flatten
...
""" Rollout Sample
"""
class CustomRecurrentRolloutBufferSamples(NamedTuple):
...
""" Rollout Buffer
"""
class CustomRolloutBuffer(RecurrentRolloutBuffer):
    ...
    def __init__(
        ...
        super().__init__(buffer_size, observation_space, action_space, hidden_state_shape, device, gae_lambda, gamma, n_envs)
    ...
    def add(self, *args,  next_obs:np.ndarray, control_action:np.ndarray, **kwargs) -> None:
        self.next_observations[self.pos] = np.array(next_obs).copy()
        self.control_actions[self.pos] = np.array(control_action).copy()
        super().add(*args, **kwargs)


    def get(self, batch_size: Optional[int] = None) -> Generator[CustomRecurrentRolloutBufferSamples, None, None]:
    ...
    def _get_samples(
    ...
    ) -> CustomRecurrentRolloutBufferSamples:
        ...
        return CustomRecurrentRolloutBufferSamples(
            ...
            control_actions=self.pad(self.control_actions[batch_inds]).reshape((padded_batch_size,) + self.control_actions.shape[1:]),
...
class CustomRecurrentPPO(RecurrentPPO):
    ...
    def _setup_model(self) -> None:
        ...
        lstm = lstm = self.policy.lstm_actor
    ...
    def collect_rollouts(
        ...
        """
        assert isinstance(
            rollout_buffer, (RecurrentRolloutBuffer, RecurrentDictRolloutBuffer, CustomRolloutBuffer)
